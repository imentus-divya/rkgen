...
use std::debug::print;
 use aptos_std::table;
    ...
    const EINVALID_ROLE: u64 = 2;
    const EALREADY_EXIST: u64 = 3;
    const EROLE_NOT_EXIST: u64 = 4;
    const EINVALID_ARGUMENTS: u64 = 5;
    ...
    public fun get_minter(): address acquires MintingManager {
        borrow_global_mut<MintingManager>(@rKGenAdmin).minter
    }
    ...
    public fun get_whitelisted_sender(): vector<address> acquires SenderWhiteList {
        borrow_global_mut<SenderWhiteList>(@rKGenAdmin).sender_vec
    }
    ...
    public fun get_whitelisted_receiver(): vector<address> acquires ReceiverWhiteList {
        borrow_global_mut<ReceiverWhiteList>(@rKGenAdmin).receiver_vec
    }
    ...
    // Event triggered when tokens are transferred from treasury to another address.
    #[event]
    struct TransferFromTreasury has drop, store {
    ...
    inline fun authorized_borrow_transfer_refs(
        ...
        // assert!(verifyTreasuryAddress(owner), error::invalid_argument(EINVALID_ROLE));
        let ref = borrow_global<ManagedRKGenAsset>(object::object_address(&asset));
        &ref.transfer_ref
    }
    ...
    fun transfer(from: &signer, to: &address, amount: u64) acquires ManagedRKGenAsset {
    ...
    public entry fun mint(
        ...
        assert!(verifyTreasuryAddress(&to), error::invalid_argument(EINVALID_ROLE));
    ...
    // Implement function to transfer rKGen from treasury to whitelist sender
    // Only treasury_address can transfer
    public entry fun transfer_from_treasury(
        treasury_address: &signer, receiver: address, amount: u64
    ) acquires ManagedRKGenAsset, TreasuryAddresses, SenderWhiteList {
        assert!(
            verifyTreasuryAddress(&signer::address_of(treasury_address)),
            error::invalid_argument(EROLE_NOT_EXIST)
        );
        assert!(
            verifySenderAddress(&receiver),
            error::invalid_argument(EROLE_NOT_EXIST)
        );

        transfer(treasury_address, &receiver, amount);
        event::emit(
            TransferFromTreasury {
                from_treasury: signer::address_of(treasury_address),
                receiver,
                amount
            }
        );
    ...
    public entry fun transfer_from_whitelist_sender(
        ...
        transfer(sender_address, &receiver, amount);
        event::emit(
